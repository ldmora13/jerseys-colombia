  ...
  useEffect(() => {
    const fetchCamisetas = async () => {
      const { data, error } = await supabase
        ...
        .order('fecha_indexacion', { ascending: false })
        .limit(3)
      if (error) {
    ...
    <div>
      ...
      <main className="bg-[#E8E8E8] flex items-center justify-start h-full w-full p-[20px]">
        <div className="flex flex-row justify-center items-start gap-10 w-full">        
          {/* Contenedor 1 */}
          <div onClick={() => navigate("/futbol")} className="flex flex-col items-center justify-center w-[300px] h-[400px] rounded-2xl bg-[#e0e0e0]" style={{ boxShadow: '15px 15px 30px #bebebe, -15px -15px 30px #ffffff' }}>
            <p className="-mt-4 mb-2 text-center text-[16px] font-semibold">Fútbol</p>
            <div className="flex flex-col gap-4 overflow-y-auto">
              {camisetas.map((camiseta, index) => {
                ...
                return (
                  <div key={index} className="flex flex-row items-center border rounded shadow p-2 gap-2 max-w-[350px] mx-auto">
                    {imagenPrincipal && (
                      <img
                        src={imagenPrincipal}
                        alt={camiseta.nombre}
          ...
          {/* Contenedor 2 */}
          <div onClick={() => navigate("/NBA")} className="flex flex-col items-center justify-center  w-[300px] h-[400px] rounded-2xl bg-[#e0e0e0]" 
              style={{ boxShadow: '15px 15px 30px #bebebe, -15px -15px 30px #ffffff' }}>
            <p className="-mt-4 mb-2 text-center text-[16px] font-semibold">NBA</p>
            {/* Aquí puedes colocar otro contenido más adelante */}
          </div>

          {/* Contenedor 3 */}
          <div className="flex flex-col items-center justify-center  w-[300px] h-[400px] rounded-2xl bg-[#e0e0e0]" 
              style={{ boxShadow: '15px 15px 30px #bebebe, -15px -15px 30px #ffffff' }}>
            <p onClick={() => navigate("/F1")} className="p-2 text-center text-[16px]">F1</p>
            {/* Aquí también puedes poner contenido personalizado luego */}
          </div>
        </div>
      </main>
      <footer className=""> 
